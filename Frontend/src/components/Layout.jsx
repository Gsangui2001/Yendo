...
// ── Íconos SVG (stroke, estilo lucide) ──────────────────────────────────────
const Icon = ({ d, fill }) => (
    ...
    <aside className="w-60 bg-white border-r border-gray-100 flex flex-col h-full">
        ...
        {navItems.map(item => (
          <button key={item.key} onClick={() => { setPage(item.key); setMobileOpen(false); }}
            className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold transition-colors
              ${page === item.key ? 'bg-green-50 text-green-700' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-800'}`}>
      ...
      <div className="px-3 pb-3">
        <div className="bg-green-50 rounded-2xl p-4">
          ...
          <button className="w-full bg-green-600 text-white text-sm font-bold py-2 rounded-xl hover:bg-green-700 transition-colors">
            Invitar ahora
          </button>
        </div>
      </div>
      ...
      <div className="px-3 pb-4 border-t border-gray-100 pt-3">
        ...
        <button onClick={() => supabase.auth.signOut()}
          className="w-full flex items-center gap-3 px-3 py-2 mt-1 rounded-xl text-sm font-semibold text-gray-500 hover:bg-red-50 hover:text-red-500 transition-colors">
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        {/* Header mobile */}
        <header className="lg:hidden bg-white border-b border-gray-100 px-4 h-14 flex items-center justify-between sticky top-0 z-40">
