...
export default function Pedido({ comercioId }) {
  ...
  function handleCliente(e) {
    ...
    setErrors(prev => ({ ...prev, clienteId: '', direccion: '' }));
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-start justify-center p-4 pt-8">
      ...
      {toast && (
        <div
          className={`fixed top-4 right-4 z-50 flex items-center gap-2 px-5 py-3 rounded-xl shadow-lg text-white text-sm font-semibold transition-all ${
        ...
        <form onSubmit={handleSubmit} noValidate className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-5">
          ...
          <div className="flex gap-3 pt-1">
            ...
            <button
              type="submit"
              disabled={loading}
              className="flex-[2] py-3 rounded-xl bg-green-600 text-white text-sm font-bold hover:bg-green-700 active:scale-95 transition-all disabled:opacity-60 disabled:cursor-not-allowed"
            >
...
function inputCls(error) {
  return `w-full px-4 py-3 rounded-xl border text-sm bg-white transition-colors
